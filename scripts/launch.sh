...
# --- CONFIGURATION (UPDATED FOR LIVING IN /SCRIPTS) ---
...
# 4. Starting Node.js server with the environment parameters loaded
echo "Starting Node.js server..."

if command -v node >/dev/null 2>&1; then
    # Reads from the saved configured profile automatically inside /server
    PORT=$PORT node --env-file="$DIR/server/.env" "$DIR/server/server.js" &
    NODE_PID=$!
